...
Callback dari Google. Membuat/update user di DB, generate JWT, lalu **redirect 302 ke `{FRONTEND_URL}/login?token=...&name=...&email=...&avatar=...`** (token dibawa via query param, disimpan client-side oleh frontend).
Error ditampilkan sebagai JSON `{ error: { code: "AUTH_FAILED", message } }`.
